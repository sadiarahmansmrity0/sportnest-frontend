...
export default function BookingModal({
  ...
  const handleBookingSubmit = async (e) => {
    ...
    try {
      ...
      // FIXED: Swapped out double quotes for backticks and added the missing closing curly brace
    ...
    <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-slate-900 border border-white/10 rounded-2xl w-full max-w-md overflow-hidden">
        ...
        <div className="p-6 border-b border-white/10 flex items-center justify-between">
          <div>
            <h2 className="text-2xl font-black text-white">Book Facility</h2>
        ...
        <form onSubmit={handleBookingSubmit} className="p-6 space-y-5">
          ...
          {/* DATE */}
          <div>
            <label className="block text-sm mb-2 text-slate-300">Select Date</label>
            <input
              type="date"
              required
          ...
          {/* SLOT */}
          <div>
            <label className="block text-sm mb-2 text-slate-300">Select Time Slot</label>
            <div className="grid gap-2 max-h-44 overflow-y-auto">
              ...
              {!facility.availableSlots?.length && (
                <p className="text-xs text-slate-500 italic py-2">No predefined slots available for this arena.</p>
              )}
            </div>
          </div>
          ...
          <div className="flex gap-3 pt-2">
            <button
              type="button"
              onClick={onClose}
              className="flex-1 bg-slate-800 hover:bg-slate-700 text-white py-3 rounded-xl font-bold transition"
            >
              Cancel
            </button>
            <button
              type="submit"
              disabled={loading}
              className="flex-1 bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-black py-3 rounded-xl transition disabled:bg-slate-800 disabled:text-slate-500"
            >
