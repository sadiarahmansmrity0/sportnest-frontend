...
export default function LoginPage() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      // We will link this to better-auth (authClient.signIn.email()) when the server is fully running
      toast.success("Login successful! Welcome back.");
      
      setTimeout(() => {
        router.push("/"); // Navigates the user back to Home
      }, 1500);
    } catch (error) {
  ...
  return (
    <div className="min-h-[80vh] flex items-center justify-center px-4 py-12 sm:px-6 lg:px-8">
      <div className="max-w-md w-full space-y-8 bg-white p-8 rounded-2xl border border-gray-200 shadow-sm">
        ...
        {/* Toggle link to Registration view */}
